...
3. Распакуйте его в любое место, запустите `AliceCoopLauncher.exe`, выберите
   папку с `AliceMadnessReturns.exe` и нажмите **Install / Repair**.
4. На компьютере хоста выберите адрес доверенной локальной сети/VPN, при
   необходимости разрешите сервер в брандмауэре и нажмите **Start Host**.
   Передайте показанный адрес второму игроку.
5. На втором компьютере введите этот адрес, нажмите **Test connection**, затем
   **Join Session**. Для найденной Steam-версии игра запускается через Steam;
...
Очищенный `drop-in.zip` и ручные BAT-файлы также остаются для опытных
пользователей. Подробности находятся в
[инструкции по установке](docs/INSTALL_RU.md).
