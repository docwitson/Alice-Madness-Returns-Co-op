...
<p align="center">
  <strong><a href="https://www.youtube.com/watch?v=QO0yarqS1-k">▶ ПОСМОТРЕТЬ ПОЛНЫЙ РЕЛИЗНЫЙ ТРЕЙЛЕР НА YOUTUBE</a></strong>
</p>
...
4. Откройте `AliceCoop\AliceCoop-LaunchConfig.bat` и укажите в `SERVER_IP` адрес
   компьютера, на котором будет работать сервер. Для двух окон на одном ПК
   используйте `127.0.0.1`.
5. На компьютере сервера/хоста запустите:
...
6. На втором компьютере запустите:
...
Обычные параметры запуска находятся в:
