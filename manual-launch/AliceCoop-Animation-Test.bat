...
set "ALICECOOP_ANIMATION_COMPARE_TRACE=1"
set "ALICECOOP_ANIMATION_TRACE=1"
set "ALICECOOP_VFX_TRACE=1"
set "ALICECOOP_CONTROL_TRACE=0"
set "ALICECOOP_ACTION_TRACE=0"
set "ALICECOOP_WORLD_TRACE=0"

echo AliceCoop animation comparison via %SERVER_IP%:%COOP_PORT%
echo LOCAL/REMOTE animation timing and focused VFX lifecycle traces are enabled.
echo Start AliceCoop-Server.bat first if the relay is not already running.
echo.

set "ALICECOOP_ROLE=host"
set "ALICECOOP_WINDOW_X=0"
echo Starting animation-test HOST...
start "AliceCoop Animation HOST" /D "%GAME_DIR%" "%GAME_EXE%" -windowed
...
echo Starting animation-test CLIENT...
start "AliceCoop Animation CLIENT" /D "%GAME_DIR%" "%GAME_EXE%" -windowed
